...
vlBool CVTFFile::Create(vlUInt uiWidth, vlUInt uiHeight, vlUInt uiFrames, vlUInt uiFaces, vlUInt uiSlices, VTFImageFormat ImageFormat, vlBool bThumbnail, vlBool bMipmaps, vlBool bNullImageData)
{
	...
	this->Header->Flags = (this->GetImageFormatInfo(ImageFormat).uiAlphaBitsPerPixel == 1 ? (vlUInt)TEXTUREFLAGS_ONEBITALPHA : 0)
							| (this->GetImageFormatInfo(ImageFormat).uiAlphaBitsPerPixel > 1 ? (vlUInt)TEXTUREFLAGS_EIGHTBITALPHA : 0)
							| (uiFaces == 1 ? 0 : (vlUInt)TEXTUREFLAGS_ENVMAP)
							| (bMipmaps ? 0 : (vlUInt)TEXTUREFLAGS_NOMIP | (vlUInt)TEXTUREFLAGS_NOLOD);
		...
		if(this->GetSupportsResources())
		{
			...
			default:
				for(vlUInt i = 0; i < this->Header->ResourceCount; i++)
				{
					if(this->Header->Resources[i].Type == uiType)
					{
						if(this->Header->Resources[i].S.Flags & RSRCF_HAS_NO_DATA_CHUNK)
						{
		...
		if(this->GetSupportsResources())
		{
			...
			default:
				...
				if(uiSize != 0)
				{
					...
					if(this->Header->Resources[uiIndex].S.Flags & RSRCF_HAS_NO_DATA_CHUNK)
					{
						if(uiSize != sizeof(vlUInt))
						{
							LastError.Set("Resources with no data chunk must have size 4.");
...
vlBool ConvertTemplated(const vlByte *lpSource, vlByte *lpDest, vlUInt uiWidth, vlUInt uiHeight, const SVTFImageConvertInfo& SourceInfo, const SVTFImageConvertInfo& DestInfo)
{
	...
	{
		const vlByte* lpFPSource = lpSource;
...
vlBool CVTFFile::Convert(const vlByte *lpSource, vlByte *lpDest, vlUInt uiWidth, vlUInt uiHeight, VTFImageFormat SourceFormat, VTFImageFormat DestFormat)
{
	...
	{
		vlByte *lpTemp = 0;
		const vlByte *lpSourceRGBA = lpSource;
		vlBool bResult = vlTrue;

		// allocate temp data for intermittent conversions
		if(SourceFormat != IMAGE_FORMAT_RGBA8888)
		{
			lpTemp = new vlByte[CVTFFile::ComputeImageSize(uiWidth, uiHeight, 1, IMAGE_FORMAT_RGBA8888)];
		}
		...
		case IMAGE_FORMAT_DXT1:
		case IMAGE_FORMAT_DXT1_ONEBITALPHA:
			bResult = CVTFFile::DecompressDXT1(lpSource, lpTemp, uiWidth, uiHeight);
			lpSourceRGBA = lpTemp;
			break;
		case IMAGE_FORMAT_DXT3:
			bResult = CVTFFile::DecompressDXT3(lpSource, lpTemp, uiWidth, uiHeight);
			lpSourceRGBA = lpTemp;
			break;
		case IMAGE_FORMAT_DXT5:
			bResult = CVTFFile::DecompressDXT5(lpSource, lpTemp, uiWidth, uiHeight);
			lpSourceRGBA = lpTemp;
			break;
		default:
...
//
vlVoid CVTFFile::ComputeImageReflectivity(const vlByte *lpImageDataRGBA8888, vlUInt uiWidth, vlUInt uiHeight, vlSingle &sX, vlSingle &sY, vlSingle &sZ)
{
	sX = sY = sZ = 0.0f;
